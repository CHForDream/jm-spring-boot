...
public class BattleDupResultLog extends AbstractLog {
	private int zero;// (占位参数)
	private int targetId;
	private int result; // 战斗结果 1胜利 2失败
	private List<ItemVo> rewards = new ArrayList<ItemVo>(); // 结算奖励
	private int stepCost; // 使用步数
	private int dupType; // 副本类型
	...
	public BattleDupResultLog(UserBean userbean) {
		super(LogfType.BATTLE_DUP_COMPLETE_APP, userbean);
	}
	...
	public String getContent() {
		...
		sb.append(super.getContent()).append("|").append(zero).append("|").append(targetId).append("|").append(result).append("|")
				.append(ItemUtil.toInfoString(rewards)).append("|").append(stepCost).append("|").append(dupType).append("|").append(leftStep).append("|")
				.append(totalStep).append("|").append(skillNum).append("|").append(getLogForTarget(leftTargets)).append("|")
				.append(getLogForTarget(totalTargets)).append("|").append(getLogForListItem(preItems)).append("|").append(getLogForListItem(battleItems))
				.append("|").append(getContinueInfo()).append("|").append(heroId).append("|").append(dupStar);
		return sb.toString();
	}
