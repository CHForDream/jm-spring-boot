 ...
 *
 */
public class ActivityBattleSumController extends ActivityBaseHandler {

	public boolean onCheck(int completeNum, long uid) {
		return true;
//		if (GameGlobals.activityManager.getOpenActivityByType(ActivityManager.ACTIVITY_TYPE_BATTLE_SUM) == null) {
//			return false;
//		}
//		if (completeNum <= 0) {
//			return false;
//		}
//		ActivityEntity activityEntity = GameGlobals.activityManager.getOpenActivityByType(ActivityManager.ACTIVITY_TYPE_BATTLE_SUM);
...
//		int newBattleSum = recordEntity.getBattleSumNum() + completeNum;
//		String record = recordEntity.getBattleSumRecord();
//		for (ActivityRewardsEntity rewardsEntity : rewardsEntities) {
//			if (rewardsEntity.getTarget() <= newBattleSum) {
...
//							ActivityManager.ACTIVITY_TYPE_BATTLE_SUM, user);
//				}
//			}
//		}
...
//		try {
//			ActivityRecordEntity recordEntity = GameGlobals.activityManager.getActivityRecordDB(uid);
//			recordEntity.setBattleSumNum(recordEntity.getBattleSumNum() + num);
//			String record = recordEntity.getBattleSumRecord();
//			for (long rewardId : rewardIds) {
//				record += rewardId + "," + ActivityManager.CAN_REWARD + ";";
//			}
//			recordEntity.setBattleSumRecord(record);
...
//			ActivityRecordEntity recordEntity = GameGlobals.activityManager.getActivityRecordDB(uid);
//			ActivityEntity activityEntity = GameGlobals.activityManager.getOpenActivityByType(ActivityManager.ACTIVITY_TYPE_BATTLE_SUM);
//			if (activityEntity == null || activityEntity.getId() != recordEntity.getBattleSumActivityId()) {
//				builder.setStatus(ActivityManager.ACTIVITY_CLOSE);
//				return;
//			}
//
//			String status = recordEntity.getBattleSumRecord();
...
//											ActivityManager.ACTIVITY_TYPE_BATTLE_SUM, rewardsEntity.getRewards(), user);
//								}
//								break loop2;
//							}
//						}
...
//					newStates += s + ";";
//				}
//				recordEntity.setBattleSumRecord(newStates);
//				Globals.getEntityProxy().updateAsync(recordEntity);
//			}
//			builder.setType(ActivityManager.ACTIVITY_TYPE_BATTLE_SUM);
...
//		ActivityRecordEntity recordEntity = GameGlobals.activityManager.getActivityRecordDB(uid);
//		return recordEntity.getBattleSumRecord();
		return null;
	}
...
//			try {
//				ActivityEntity activityEntity = GameGlobals.activityManager.getOpenActivityByType(ActivityManager.ACTIVITY_TYPE_BATTLE_SUM);
//				if (activityEntity == null || activityEntity.getId() != activityRecordEntity.getBattleSumActivityId()) {
//					activityRecordEntity.setBattleSumActivityId(activityEntity == null ? 0 : activityEntity.getId());
//					activityRecordEntity.setBattleSumRecord("");
//					activityRecordEntity.setBattleSumNum(0);
